...
设置页分别显示 Windows 和 macOS 的博客项目目录。Windows 电脑选择 Windows 仓库，macOS 电脑选择 macOS 仓库；发布时插件按当前系统自动使用对应目录。目录名称会随思源设置同步，实际目录授权保存在当前电脑的本机浏览器存储里，不会随思源配置同步。每台电脑只需选择一次；如果系统之后再次询问，请允许读写。插件不需要读取或保存绝对路径，也不需要启动 Bridge。
...
- Windows 和 macOS 各自单独选择对应电脑上的博客项目根目录；发布时按当前系统自动路由到相应目录。
